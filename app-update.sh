...
az containerapp update -n "${sourceapp}-local" -g $rg  --source $sourcepath --subscription $subscription

echo 'Deploy source code from git repo with container registery'
az containerapp update -n "${repoapp}" -g $rg  --subscription $subscription