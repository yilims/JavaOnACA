...
#az containerapp delete -n "${sourceapp}-repo" -g $rg --subscription $subscription
#az containerapp up -n "${sourceapp}-repo" -g $rg  --environment $enviroment --source $sourcepath --ingress external --target-port 8080 --subscription $subscription --location $location --registry-server $registryserver --registry-user $registryuser --registry-pass $registerpass


# echo 'Deploy with source code with repo'
# #az containerapp delete -n "${sourceapp}-repo" -g $rg --subscription $subscription
# az containerapp up -n "${sourceapp}-repo" -g $rg  --environment $enviroment --repo "https://github.com/showpune/spring-petclinic" --ingress external --target-port 8080 --subscription $subscription --location $location --registry-server $registryserver --registry-user $registryuser --registry-pass $registerpass

# echo 'Deploy with source code with repo'
# #az containerapp delete -n "${repoapp}" -g $rg --subscription $subscription
# az containerapp up -n "${repoapp}" -g $rg  --environment $enviroment --repo $repourl --ingress external --target-port 8080 --subscription $subscription --location $location 

echo 'Deploy with source code with repo no registery'
#az containerapp delete -n "${repoappnoreg}"-g $rg --subscription $subscription
